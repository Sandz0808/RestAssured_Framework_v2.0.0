...
def load_results(results_directory: Path) -> list[dict[str, Any]]:
    ...
    for result_file in sorted(results_directory.rglob("*-result.json")):
        try:
            ...
            print(f"Skipping invalid result file {result_file}: {error}")
            continue
        ...
        results.append(
            {
                ...
                "labels": labels,
                "status": status,
                "logical_key": logical_test_key(result, labels),
                "retry_key": retry_group_key(result, labels),
...
def consolidate_retries(
    ...
    for attempts in groups.values():
        ...
        prior_statuses = {
            attempt["status"] for attempt in attempts[:-1]
        }
        status_details = final_attempt["raw"].get("statusDetails") or {}

        final_attempt["retry_count"] = max(0, len(attempts) - 1)
        final_attempt["is_flaky"] = bool(status_details.get("flaky")) or (
                final_attempt["status"] == "passed"
                and bool(
            prior_statuses.intersection(
                {"failed", "broken", "unknown"}
            )
        )
        )
...
def to_iso_timestamp(epoch_milliseconds: Any) -> str | None:
...
def build_test_record(result: dict[str, Any]) -> dict[str, Any]:
    raw = result["raw"]
    ...
    if start_ms is not None and stop_ms is not None:
        try:
            duration_ms = max(0, int(stop_ms) - int(start_ms))
        except (TypeError, ValueError):
            duration_ms = None

    status_details = raw.get("statusDetails") or {}

    return {
        "historyId": raw.get("historyId"),
        "testCaseName": raw.get("name") or "Unnamed test",
        ...
        "startedAt": to_iso_timestamp(start_ms),
        "completedAt": to_iso_timestamp(stop_ms),
        "durationMs": duration_ms,
        "retryCount": result["retry_count"],
        "isFlaky": result["is_flaky"],
        "failureMessage": status_details.get("message"),
    }
...
def build_summary(
    ...
    status_counts = Counter(
        result["status"] for result in final_results
    )
    ...
    if status_counts["failed"] > 0 or status_counts["broken"] > 0:
        overall_status = "FAILED"
    elif status_counts["unknown"] > 0:
        overall_status = "UNKNOWN"
    elif status_counts["skipped"] == len(final_results) and final_results:
    ...
    return {
        ...
        "run": {
            ...
            "uniqueTestCases": len(
                {result["logical_key"] for result in final_results}
            ),
...
def main() -> int:
    ...
    output_file = (
        Path(sys.argv[2])
        if len(sys.argv) == 3
        else Path("target") / "test-summary.json"
    )

    if not results_directory.is_dir():
        print(f"Allure results directory not found: {results_directory}")
        return 2

    raw_results = load_results(results_directory)
    ...
    if not raw_results:
        ...
        final_results = consolidate_retries(raw_results)
        summary = build_summary(raw_results, final_results)
    ...
    print(f"Test summary generated successfully: {output_file}")
    print(
        f"Status: {summary['run']['status']} | "
        f"Total: {summary['run']['totalTests']} | "
        f"Passed: {summary['run']['passed']} | "
        f"Failed: {summary['run']['failed']} | "
        f"Broken: {summary['run']['broken']} | "
        f"Skipped: {summary['run']['skipped']}"
    )
